...
Feel free to check out the result of the above code block by logging out the value of `command` and `args`. You may want to add a little more logic to make this resilient to malformed input. But we will leave that exercise up to you.
...
First, we will import the `fs` module at the top of the file just before `var stdin = process.openStdin();`. The `fs` module is the Node.js core module for file system operations.
