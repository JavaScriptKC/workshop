...
Similarly to the `fs` module, Node.js core contains a `http` module which can be used to act as a HTTP client or server. In the next lab you will be creating a HTTP server, but for now, we'll focus on creating a simple `wget` command to download a file.
